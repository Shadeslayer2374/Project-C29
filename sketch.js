const Engine = Matter.Engine;
const World= Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
const Render = Matter.Render;

var engine, world;



function setup(){
    var canvas = createCanvas(1000,550);
    engine = Engine.create();
    world = engine.world;

    gr1 = new Ground(650,540,1300,15);
    gr2 = new Ground(640,400,200,30);
    gr3 = new Ground(400,450,200,30);
    gr4 = new Ground(860,350,200,30);
    gr5 = new Ground(100,500,200,100);
    ball1 = new Ball(100,300,30);

    box1 = new Box(590,300);
    box2 = new Box2(640,300);
    box3 = new Box(610,250);
    box4 = new Box2(660,250);
    box5 = new Box2(350,400);
    box6 = new Box(450,400);
    box7 = new Box2(375,350);
    box8 = new Box(400,300);
    box9= new Box2(850,300);
    box10 = new Box(900,300);
    box11 = new Box2(875,250); 
    box12 = new Box(850,200);
    sling = new SlingShot(ball1.body,{x:100, y:300});
}

function draw(){
    background(0);
    Engine.update(engine);
 
    gr1.display();
    gr2.display();
    gr3.display();
    gr4.display();
    gr5.display();
    ball1.display();  

    box1.display(); 
    box2.display();
    box3.display();
    box4.display();
    box5.display();
    box6.display();
    box7.display();
    box8.display();
    box9.display();
    box10.display();
    box11.display();
    box12.display();
    textSize(20);
    sling.display();
   // mouseDragged();
    //mouseReleased();
}

function mouseDragged(){
        Matter.Body.setPosition(ball1.body, {x: mouseX , y: mouseY});
}


function mouseReleased(){
    sling.fly();
}

function keyPressed(){
  if(keyCode===32){
    
      sling.attach(ball1.body);
  }
}